...
START_TEST(test_1) {
  ...
  int ret = s21_sum_matrix(&A, &B, &C);
  ck_assert_int_eq(ret, OK);

  matrix_t result = get_simple_matrix_sum();
...
START_TEST(test_2) {
  ...
  matrix_t result = get_matrix_12_sum();
...
Suite *sum_matrix_tests(void) {
  Suite *s = suite_create("\033[34m__SUM_MATRIX_TESTS__\033[0m");
